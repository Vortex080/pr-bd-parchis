...
select j.n_jugador, p.n_arbitro, j.nombre_competicion
from juega j join partida p on p.nombre_competicion = j.nombre_competicion
where j.nombre_competicion like upper('PARTIDA PARCHÍS LIMA') and j.jornada = p.jornada;

select * from partida where nombre_competicion like upper('PARTIDA PARCHÍS LIMA');
select * from juega where nombre_competicion like upper('PARTIDA PARCHÍS LIMA');

select j.n_jugador as v_jugador, p.n_arbitro as v_arbitro, j.nombre_competicion
        from juega j join partida p on p.nombre_competicion = j.nombre_competicion
        where j.nombre_competicion like upper('PARTIDA PARCHÍS LIMA') and j.jornada = p.jornada;
...
begin 
   ...
   loop
        ...
        loop
            ...
            select j.n_jugador, p.n_arbitro, j.nombre_competicion into v_jugado, v_arbitrando
            from juega j join partida p on p.nombre_competicion = j.nombre_competicion
...
create or replace procedure saca_jug_bueno 
as 
begin 

        for i in 1..(select max(asociado) form participantes)
        loop
